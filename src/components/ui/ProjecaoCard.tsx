...
import { calcProjecao, formatBRL } from '@/utils/store'
import type { Store } from '@/types'

export function ProjecaoCard({ store }: { store: Store }) {
  const proj = calcProjecao(store)

  const Icon  = proj.tendencia === 'alta' ? TrendingUp : proj.tendencia === 'baixa' ? TrendingDown : Minus
  const color = proj.tendencia === 'alta' ? '#22c55e'  : proj.tendencia === 'baixa' ? '#ef4444'    : '#9ca3af'
  const bg    = proj.tendencia === 'alta' ? 'rgba(34,197,94,.08)' : proj.tendencia === 'baixa' ? 'rgba(239,68,68,.08)' : 'rgba(156,163,175,.08)'
