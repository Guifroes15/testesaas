...
export function MonthFilter({ allMonths, monthLabels, selected, onChange }: Props) {
  ...
  const selectLastN = (n: number) => onChange(new Set(allMonths.slice(-n)))
  ...
  const isPeriod = (n: number) => {
    ...
    return allMonths.slice(-n).every(m => selected.has(m))
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        ...
        {[
          { label: 'Último Mês', action: () => selectLastN(1), active: selected.size === 1 && selected.has(allMonths[allMonths.length - 1]) },
      ...
      <div className="relative group">
        <div className="flex items-center gap-1.5 overflow-x-auto pb-2 no-scrollbar mask-fade-right">
          {allMonths.map((chave, i) => {
            const active = selected.has(chave)
            const label  = monthLabels[i].split('/')[0]
