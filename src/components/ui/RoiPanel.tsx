...
import { calcRoi, formatBRL } from '@/utils/store'
...
function StatCard({ label, value, sub, icon: Icon, trend }: { label: string; value: string; sub: string; icon: LucideIcon; trend?: 'up' | 'down' }) {
  return (
    <motion.div whileHover={{ y: -2 }} className="bg-brand-medium border border-brand-light p-4 rounded-xl space-y-2 group">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard label="Retorno Acumulado" value={formatBRL(roiData.roiTotal)} sub={`ROI: ${roiData.roiTotalPct.toFixed(1)}%`} icon={Target} trend={roiData.roiTotal >= 0 ? 'up' : 'down'} />
        <StatCard label="Vendas Aure"  value={formatBRL(roiData.totalVendas)} sub={`${roiData.meses.length} meses analisados`} icon={TrendingUp} />
        <StatCard label="Custo Total"  value={formatBRL(roiData.totalCusto)} sub={`Fee: ${formatBRL(roiData.totalFee)} + Mídia: ${formatBRL(roiData.totalVerba)}`} icon={DollarSign} />
        <StatCard label="Meses Lucrativos" value={`${roiData.mesesPositivos}`} sub={`de ${roiData.meses.length} meses`} icon={Wallet} />
      </div>
      ...
      <div className="flex gap-3 p-4 bg-violet-500/10 border border-violet-500/20 rounded-lg text-xs leading-relaxed text-violet-200">
        ...
        <div>
          O <strong>Custo Total</strong> é a soma da taxa mensal de gestão (FEE) de {formatBRL(fee)} + o investimento realizado em mídia (Verba) no mês.
