...
import { calcHealthScore } from '@/utils/store'
import type { Store } from '@/types'

export function HealthBadge({ store }: { store: Store }) {
            ...
            {health.detalhes.map((d, i) => (
              <div key={i}>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-white font-semibold">{d.valor} — {d.pontos}/{d.maxPontos}pts</span>
                </div>
                <div className="h-1.5 rounded-full bg-gray-800">
                  <div
                    className="h-1.5 rounded-full transition-all"
                    style={{ width: `${(d.pontos / d.maxPontos) * 100}%`, background: health.color }}
                  />
                </div>
              </div>
            ))}
          </div>
          <div className="mt-3 pt-3 flex justify-between items-center text-xs font-bold" style={{ borderTop: '1px solid #2a2a38' }}>
...
export function HealthDot({ store }: { store: Store }) {
  const health = calcHealthScore(store)
  return (
    <span
